...
function checkExportRateLimit(campaignId, actorKey) {
  ...
  if (!bucket || bucket.resetAt <= now) {
    ...
    return {
      allowed: true,
      remaining: EXPORT_RATE_LIMIT_MAX - 1,
  ...
  return {
    allowed: true,
    remaining: EXPORT_RATE_LIMIT_MAX - bucket.count,
 ...
 * }} options
 */
export function createCampaignExportRoute({
  db,
  campaignRepository,
  auditLogRepository,
  requireApiKey,
  ...
  router.get('/campaigns/:id/export', requireApiKey, async (req, res) => {
    ...
    if (format !== 'csv' && format !== 'json') {
      return res
        .status(400)
        .json({ error: 'Invalid format. Use ?format=csv or ?format=json', code: 'INVALID_FORMAT' });
    }
    ...
    const participants = collectParticipants(db, id, { fromDate, toDate });
    ...
    if (format === 'csv') {
      ...
      await pipeline(Readable.from([buildCsv(PARTICIPANT_COLUMNS, participants)]), res);
    } else {
