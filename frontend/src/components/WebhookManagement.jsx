...
export default function WebhookManagement() {
  ...
  const handleRotateSecret = async (id) => {
    if (!confirm('This will generate a new secret. The old secret will no longer work. Continue?')) return;
  ...
  const handleReplayDelivery = async (webhookId, deliveryId) => {
    try {
      const response = await fetch(`/api/v1/webhooks/${webhookId}/deliveries/${deliveryId}/replay`, {
        method: 'POST',
      });
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      {error && (
        <div style={{ background: '#fee2e2', border: '1px solid #fecaca', padding: '12px', borderRadius: '8px', marginBottom: '16px', color: '#991b1b' }}>
          {error}
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '24px' }}>
        ...
        <div>
          ...
          {webhooks.length === 0 ? (
            <div style={{ padding: '24px', background: '#f1f5f9', borderRadius: '8px', textAlign: 'center', color: '#64748b' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {webhooks.map(webhook => (
                ...
                >
                  <div style={{ fontWeight: 600, marginBottom: '4px' }}>{webhook.description || webhook.url}</div>
          ...
          {selectedWebhook ? (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                <h2 style={{ fontSize: '1.1rem', margin: 0 }}>{selectedWebhook.description || 'Webhook Details'}</h2>
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button
                    onClick={() => handleRotateSecret(selectedWebhook.id)}
                    style={{ padding: '6px 12px', background: '#f59e0b', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem' }}
                  >
                    Rotate Secret
                  </button>
                  <button
                    onClick={() => handleDeleteWebhook(selectedWebhook.id)}
                    style={{ padding: '6px 12px', background: '#ef4444', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem' }}
                  >
                    Delete
                  </button>
                </div>
              ...
              <div style={{ background: '#f8fafc', padding: '16px', borderRadius: '8px', marginBottom: '16px' }}>
                <div style={{ marginBottom: '8px' }}>
                  <strong>URL:</strong>
                  <div style={{ fontFamily: 'monospace', fontSize: '0.9rem', wordBreak: 'break-all' }}>{selectedWebhook.url}</div>
                </div>
                <div style={{ marginBottom: '8px' }}>
                  <strong>Events:</strong>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '4px' }}>
                    {selectedWebhook.events.map(event => (
                      <span key={event} style={{ background: '#e0e7ff', padding: '4px 8px', borderRadius: '4px', fontSize: '0.8rem' }}>
                        {event}
                      </span>
                    ))}
                  </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                ...
                <button
                  ...
                  style={{ padding: '6px 12px', background: '#10b981', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem' }}
                >
                  Test Webhook
                </button>
              </div>
              ...
              {deliveryLogs.length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {deliveryLogs.map(log => (
                    <div key={log.id} style={{ padding: '12px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '6px' }}>
                      ...
                      {log.status === 'failed' && (
                        <button
                          ...
                          style={{ marginTop: '8px', padding: '4px 8px', background: '#6366f1', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontSize: '0.8rem' }}
                        >
                          Replay
                        </button>
                      )}
            ...
            <div style={{ padding: '48px', background: '#f1f5f9', borderRadius: '8px', textAlign: 'center', color: '#64748b' }}>
      ...
      {showCreateModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', padding: '24px', borderRadius: '12px', maxWidth: '500px', width: '90%' }}>
            ...
            <form onSubmit={handleCreateWebhook}>
              ...
              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', marginBottom: '4px', fontWeight: 600 }}>Events</label>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {AVAILABLE_EVENTS.map(event => (
                    <label key={event} style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.9rem' }}>
                      <input
                        type="checkbox"
              ...
              <div style={{ marginBottom: '16px' }}>
                <label style={{ display: 'block', marginBottom: '4px', fontWeight: 600 }}>Secret (optional)</label>
                <input
                  type="text"
              ...
              <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end' }}>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  style={{ padding: '8px 16px', background: '#6366f1', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', disabled: loading ? 'opacity: 0.5' : undefined }}
                >
      ...
      {showTestModal && selectedWebhook && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: 'white', padding: '24px', borderRadius: '12px', maxWidth: '500px', width: '90%' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', marginBottom: '4px', fontWeight: 600 }}>Event Type</label>
              <select
                value={testEvent}
            ...
            <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', marginBottom: '16px' }}>
              <button
                ...
                disabled={loading}
                style={{ padding: '8px 16px', background: '#10b981', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', disabled: loading ? 'opacity: 0.5' : undefined }}
              >
            ...
            {testResult && (
              <div style={{ background: testResult.success ? '#dcfce7' : '#fee2e2', padding: '12px', borderRadius: '6px', fontSize: '0.9rem' }}>
                <div style={{ fontWeight: 600, marginBottom: '4px' }}>
                  {testResult.success ? '✓ Test Successful' : '✗ Test Failed'}
                </div>
                {testResult.statusCode && <div>Status Code: {testResult.statusCode}</div>}
                {testResult.response && <div style={{ marginTop: '4px', fontFamily: 'monospace', fontSize: '0.8rem' }}>{testResult.response}</div>}
